...
class MobileDumper(DiscoverableTransform):
    ...
    def get_call_logs(dataDir):
        calls_db = os.path.join(dataDir, "data", "com.android.providers.contacts", "databases", "calllog.db")  # Adjusted to calllog.db
        calls = []
        try:
    ...
    def get_chrome_history(dataDir):
        history_db = os.path.join(dataDir, "data", "com.android.chrome", "app_chrome", "Default", "History")  # Updated path
        history = []
        try:
